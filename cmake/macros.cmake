...
##############################################################################
 # add_lcv_library(<library_name>, [<path>])
 # 
 # == Description == 
...
##############################################################################
macro(add_lcv_library library_name)
	set(library_directory ${library_name})
	if(${ARGC} EQUAL 2)
		set(library_directory ${ARGV1})
	endif(${ARGC} EQUAL 2)
	file(GLOB sources
		"${library_directory}/src/*.cpp"
		"${library_directory}/src/*.c"
	)
	add_library(${library_name} STATIC ${sources})
	
	include_directories(BEFORE "${library_directory}/include")
endmacro(add_lcv_library)

##############################################################################
 # add_lcv_executable(<executable_name>, [<path>])
 # 
 # == Description == 
 # This macro adds cpp sources and include files for a static library and sets 
 # up a build target for this library named <library_name>. 
 ...
 # headers in <library_name>/include
##############################################################################
macro(add_lcv_executable executable_name)
...
##############################################################################
 # lcv_rospack_init(<name> <gen_messages> <gen_services>)
 # 
 # == Description == 
...
##############################################################################
macro(lcv_rospack_init name gen_messages gen_services)
...
##############################################################################
macro(lcv_rospack_end)
	set(PROJECT_SOURCE_DIR ${TEMPORARY_PROJECT_SOURCE_DIR})
endmacro(lcv_rospack_end)

##############################################################################
 # lcv_cannot_build(<target_name> <friendly_name>)
 # 
 # == Description == 
...
##############################################################################
macro(lcv_cannot_build target_name friendly_name)
	message(STATUS "${friendly_name} can't be build because libraries are missing. ") 
endmacro(lcv_cannot_build)

##############################################################################
 # lcv_check_ros()
 # 
 # == Description == 
 # This macro checks if rosbuild can be found and checks if lcv_check_ros_path()
...
macro(lcv_check_ros_path)
	...
	if(NOT ${listlen} GREATER 1) 
		message(WARNING "ROS_PACKAGE_PATH needs to contain both the packages in the lcv repository and the default paths. It should be configured in .bashrc after including ROS ssetup.bash. The rule should look like this: \n export ROS_PACKAGE_PATH=/home/username/gitrepos/low-cost-vision-2012/ROS:$ROS_PACKAGE_PATH\n")
		break()
...
endmacro(lcv_check_ros_path)


##############################################################################
 # add_lcv_doc(<doc_name>)
 # 
 # == Description == 
 # This macro generates a documentation target
##############################################################################
macro(add_lcv_doc doc_name)
	if(DOXYGEN_FOUND)
