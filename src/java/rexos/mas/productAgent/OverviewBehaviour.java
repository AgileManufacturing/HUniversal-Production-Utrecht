		...
		_sequentialBehaviour.addSubBehaviour(new OneShotBehaviour() {
			...
			public void action() {
				...
				for (ProductionStep stp : _productAgent.getProduct().getProduction().getProductionSteps()) {
					...
					for (AID aid : _productAgent.getProduct().getProduction().getProductionEquipletMapping().getEquipletsForProductionStep(stp.getId()).keySet()) {
						System.out.println("Eq localname: " + aid.getLocalName() + " AID: " + aid + " timeslots: " + _productAgent.getProduct().getProduction().getProductionEquipletMapping().getTimeSlotsForEquiplet(stp.getId(), aid));
					}
					System.out.println("\n");
				}
			}
