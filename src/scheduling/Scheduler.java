...
public class Scheduler{
	public Scheduler(AID[] equipletList, Object productionStep)
		...
		for(int i = 0; i < equipletList.length; i++){
			// old name is eq1
			List<DBObject> data = db.getCollection(equipletList[i].getName())
					.find().toArray();// nameOfCollection should be 'schedule'
		...
		for(int extract = 0; extract < equipletList.length; extract++){
			List<DBObject> data = db
					.getCollection(equipletList[extract].getName()).find()
		...
		for(int i = 0; i < equipletList.length; i++){
			if (equipletList[i].getName().equals(
					freetimeslotEq.getEquipletName())){
