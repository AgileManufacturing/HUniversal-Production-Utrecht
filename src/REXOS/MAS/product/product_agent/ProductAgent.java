...
public class ProductAgent extends Agent{
	...
	protected void setup(){
		...
		if (arguments.length <= 0){
			...
			try {
				JSONObject productSteps = new JSONObject(new JSONTokener(arguments[0].toString()));
				...
				try {
					...
					if (msg!=null) {
						...
						if (msg.getPerformative() == MessageType.AVAILABLE_TO_PLAN){
							JSONObject proposal = new JSONObject(new JSONTokener(msg.getContent()));
							proposals.add(new Proposal(proposal,msg.getSender()));
							if (proposals.size() == 1){
								for (int i=0; i < productStepList.length(); i++) {
									String productStepId = productStepList.getJSONObject(i).getString("id");
									if (productStepId.equals(proposal.getString("productStepId"))) {
										proposal.put("productStep", productStepList.get(i));
										break;
									}
								}
							...
							supplied = true;
							JSONObject productStep = new JSONObject(new JSONTokener(msg.getContent()));
	...
	private void planCurrentProductStep() throws JSONException{
		if (productStepList.length() > currentPlannedProductStep){
			...
			ProductStep productStep = (new ProductStep(productStepList.getJSONObject(currentPlannedProductStep)));
			if(productStep.getService().getName().equals("place") && !supplied){
				String message = productStepList.getJSONObject(currentPlannedProductStep).toString();
				...
				try {
					...
					JSONObject message = productStepList.getJSONObject(currentPlannedProductStep);
