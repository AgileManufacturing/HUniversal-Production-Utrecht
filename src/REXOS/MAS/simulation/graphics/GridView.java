...
//		}
	 ...
	 * 
	 * @param equiplets
	 *            :: Map < equiplet name, Pair < position in grid, state ::
	 *            Tuple < state, # of (ready) products waiting, # of products
	 *            scheduled, # of products executed > > >
	 */
	@Deprecated
	 ...
	 *            :: List < equiplet name, position in grid, services, state ::
	 *            < state, # of (ready) products waiting, # of products
	 *            scheduled, # of products executed > >
	 */
	public void update(List<Tuple<String, Position, List<String>, Tuple<String, Integer, Integer, Integer>>> equipletStates) {
		...
		for (Tuple<String, Position, List<String>, Tuple<String, Integer, Integer, Integer>> equiplet : equipletStates) {
			...
			final EquipletView component = new EquipletView(name, services, state, waiting, scheduled, executed);
			component.setBackground(Color.WHITE);
			component.setBorder(BorderFactory.createLineBorder(Color.BLACK, 2));
