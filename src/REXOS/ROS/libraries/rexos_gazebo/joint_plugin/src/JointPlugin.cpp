...
namespace joint_plugin {
	...
	void JointPlugin::Load(physics::WorldPtr _world, sdf::ElementPtr _sdf) {
		...
		addJointServer = nodeHandle->advertiseService(rosNamespace + "/jointConstraint/addJoint", 
				&JointPlugin::addJoint, this);
		removeJointServer = nodeHandle->advertiseService(rosNamespace + "/jointConstraint/removeJoint", 
	...
	bool JointPlugin::addJoint(joint_plugin::addJoint::Request& request, 
		...
		ROS_INFO_STREAM("Adding entity " << request.model << "::" << request.joint);
