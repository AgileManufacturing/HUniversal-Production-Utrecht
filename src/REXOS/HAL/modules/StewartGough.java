...
public class StewartGough extends ModuleActor {
	...
	public ArrayList<HardwareStep> translateCompositeStep(CompositeStep compositeStep) throws ModuleTranslatingException, FactoryException {
		...
		if(compositeStep.getCommand().get(CompositeStep.RELATIVE_TO) == null){
			parameters.addProperty("ID", "null");
		}
		else{
			parameters.addProperty("ID", compositeStep.getCommand().get(CompositeStep.RELATIVE_TO).getAsString());
		}
		
		OriginPlacement originPlacement = new OriginPlacement(OriginPlacementType.RELATIVE_TO_IDENTIFIER, parameters);
