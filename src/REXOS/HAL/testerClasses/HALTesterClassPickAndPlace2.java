...
public class HALTesterClassPickAndPlace2 implements
		HardwareAbstractionLayerListener {
	static HALTesterClassPickAndPlace2 htc = new HALTesterClassPickAndPlace2();
	static HardwareAbstractionLayer hal;
	
	static int productstepnumberx = 0;
	static int productstepnumbery = 0;
	static int direction = 1;
	static String id1 , id2;
	
	static double[] xs = {-16.5, -5.5, 5.5 , 16.5};
	static double[] ys = {16.5, 5.5, -5.5 , -16.5};
	...
	public static void executeDemoStep() {
		...
		try {
			...
			targetMove1.put("x", xs[productstepnumberx]);
			targetMove1.put("y", ys[productstepnumbery]);
			...
			target1.put("identifier", id1);

			JSONArray subjects1 = new JSONArray();
			JSONObject subject1 = new JSONObject();
			JSONObject subjectMove1 = new JSONObject();
			subjectMove1.put("x", xs[productstepnumberx]);
			subjectMove1.put("y", ys[productstepnumbery]);
			...
			subject1.put("identifier", id2);
			subjects1.put(subject1);

			criteria1.put("target", target1);
			criteria1.put("subjects", subjects1);
			hal.translateProductStep(new ProductStep(1, criteria1, new Service(
					"place")));
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		productstepnumberx++;
		if (productstepnumberx > 3) {
			productstepnumberx = 0;
			productstepnumbery++;
			if(productstepnumbery > 3) {
				productstepnumbery = 0;
...
//		hal.insertModule(b, b);
	...
	public void onTranslationFinished(ProductStep productStep,
		...
		Logger.log(LogSection.NONE, LogLevel.INFORMATION,
	...
	public void onTranslationFailed(ProductStep productStep) {
		Logger.log(LogSection.NONE, LogLevel.NOTIFICATION,
				"Translation failed of the following product step:",
				productStep);
	}
	...
	public void onProcessStatusChanged(HardwareStepStatus status,
		...
		Logger.log(LogSection.NONE, LogLevel.INFORMATION, "The status of "
				+ hardwareStep + " (being processed by module " + module
	...
	public void onModuleStateChanged(String state, Module module) {
		Logger.log(LogSection.NONE, LogLevel.INFORMATION,
				"The state of module " + module + " has changed to " + state);
	}

	@Override
	public void onModuleModeChanged(String mode, Module module) {
		Logger.log(LogSection.NONE, LogLevel.INFORMATION, "The mode of module "
				+ module + " has changed to " + mode);
	}
	...
	public void onEquipletStateChanged(String state) {
		Logger.log(LogSection.NONE, LogLevel.INFORMATION,
				"The state of equiplet " + getEquipletName()
	...
	public void onEquipletModeChanged(String mode) {
		Logger.log(LogSection.NONE, LogLevel.INFORMATION,
				"The mode of equiplet " + getEquipletName()
						+ " has changed to " + mode);
	}
	...
	public void onReloadEquiplet(String state) {
		Logger.log(LogSection.NONE, LogLevel.INFORMATION, "Reloading has: "
				+ state);
	 ...
	 * 
	 * 
	 * //Bakje 3 GOED MORE TEST //double falsex = -2.2; //double falsey = 2.4;
	 * JSONObject target1 = new JSONObject(); JSONObject targetMove1 = new
	 * JSONObject(); targetMove1.put("x", 5.5); targetMove1.put("y", -5.5);
	 * targetMove1.put("z", 13.8); JSONObject targetMove1Approach = new
	 * JSONObject(); targetMove1Approach.put("x", 0);
	 * targetMove1Approach.put("y", 0); targetMove1Approach.put("z", 20);
	 * targetMove1.put("approach", targetMove1Approach);
	 * target1.put("move",targetMove1); target1.put("identifier", "GC4x4MB_1");
	 * 
	 * JSONArray subjects1 = new JSONArray(); JSONObject subject1 = new
	 * JSONObject(); JSONObject subjectMove1 = new JSONObject();
	 * subjectMove1.put("x", 5.5); subjectMove1.put("y", -5.5);
	 * subjectMove1.put("z", 13.8); JSONObject subjectMove1Approach = new
	 * JSONObject(); subjectMove1Approach.put("x", 0);
	 * subjectMove1Approach.put("y", 0); subjectMove1Approach.put("z", 20);
	 * subjectMove1.put("approach", subjectMove1Approach);
	 * subject1.put("move",subjectMove1); subject1.put("identifier",
	 * "GC4x4MB_6"); subjects1.put(subject1);
	 * 
	 * JSONObject target2 = new JSONObject(); JSONObject targetMove2 = new
	 * JSONObject(); targetMove2.put("x", 5.5); targetMove2.put("y", -5.5);
	 * targetMove2.put("z", 13.8); JSONObject targetMove2Approach = new
	 * JSONObject(); targetMove2Approach.put("x", 0);
	 * targetMove2Approach.put("y", 0); targetMove2Approach.put("z", 20);
	 * targetMove2.put("approach", targetMove2Approach);
	 * target2.put("move",targetMove2); target2.put("identifier", "GC4x4MB_6");
	 * 
	 * JSONArray subjects2 = new JSONArray(); JSONObject subject2 = new
	 * JSONObject(); JSONObject subjectMove2 = new JSONObject();
	 * subjectMove2.put("x", 5.5); subjectMove2.put("y", -5.5);
	 * subjectMove2.put("z", 13.8); JSONObject subjectMove2Approach = new
	 * JSONObject(); subjectMove2Approach.put("x", 0);
	 * subjectMove2Approach.put("y", 0); subjectMove2Approach.put("z", 20);
	 * subjectMove2.put("approach", subjectMove2Approach);
	 * subject2.put("move",subjectMove2); subject2.put("identifier",
	 * "GC4x4MB_1"); subjects2.put(subject2);
	 * 
	 * 
	 * 
	 * criteria2.put("target",target2); criteria2.put("subjects", subjects2);
	 */
