...
public class PlannerBehaviour extends CyclicBehaviour{
	...
	public void action() {
		...
		ArrayList<Long> eqCaps = new ArrayList<Long>();
		eqCaps.add(1L);
		eqCaps.add(2L);
		
		equipletCapabilities.put(new AID("eqa1",AID.ISLOCALNAME), eqCaps);
		//equipletCapabilities.p
		...
		for(ProductionStep ps : psa) {
			long id = ps.getId();
			long capability = ps.getCapability();
